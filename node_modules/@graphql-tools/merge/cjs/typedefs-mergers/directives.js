...
function collectMultiInstanceDirectiveNames(directiveNodes) {
    const seen = new Set();
    const multi = new Set();
...
function isRepeatableDirective(directive, directives, repeatableLinkImports, inferredRepeatable) {
    const directiveDef = directives?.[directive.name.value];
    if (directiveDef) {
        return !!directiveDef.repeatable;
    }
    return !!(repeatableLinkImports?.has(directive.name.value) ||
...
function mergeDirectives(d1 = [], d2 = [], config, directives) {
    ...
    const inferredRepeatable = new Set();
    ...
    for (const directive of [...asNext, ...asFirst]) {
        if (isRepeatableDirective(directive, directives, config?.repeatableLinkImports, inferredRepeatable)) {
